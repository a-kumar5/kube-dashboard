...
def lambda_handler(_event, _context):
    ...
    hpa_ret = client.AutoscalingV2Api().list_namespaced_horizontal_pod_autoscaler("default")
    ret = v1_api.list_namespaced_pod("default")
    return f"There are {len(ret.items)} pods in the default namespace.", f"There are {len(hpa_ret.items)} hpa in the default namespace."
